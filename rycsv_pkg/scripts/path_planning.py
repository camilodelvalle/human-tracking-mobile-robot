...
'main' function modified by Camilo Del Valle (@camilodelvalle) to be 
used as part of this ROS package.
...
# parameter
N_SAMPLE = 500  # number of sample_points, default = 500
...
def main():
    ...
    img = cv2.imread('paths/map.pgm',0)
    _, img = cv2.threshold(img,220,255,cv2.THRESH_BINARY)
        
    h, w = img.shape[:2]

    # Scaling, factors determined manually
    height_factor = h/14.0 # [meters]
    width_factor = w/15.0 # [meters]
    ...
    # Scaling, factors determined manually
    rows = rows/height_factor - 9.2
    cols = cols/width_factor - 7.8
    ...
    np.savetxt('paths/px.csv',rx,delimiter=',')
    np.savetxt('paths/py.csv',ry,delimiter=',')

    print('Files px.csv and py.csv saved correctly')
